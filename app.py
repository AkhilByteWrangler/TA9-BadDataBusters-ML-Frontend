...
def load_lottie_url(url: str):
    ...
    Returns:
        dict: The JSON data for the Lottie animation if successful, otherwise None.
    """
    try:
        r = requests.get(url)
...
# Sidebar instructions for users
...
# Collect user inputs for health metrics via sliders
...
# Feature engineering to add derived metrics
bmi_age = bmi * age
...
# Display prediction results with animations and color-coded sections
st.markdown('<div class="section-header">📈 Prediction Results</div>', unsafe_allow_html=True)

if prediction == 1:
    # Display high-risk result with warning animation
    if lottie_warning:
    ...
    # Display low-risk result with success animation
    if lottie_success:
...
# Gauge meter for visualizing the confidence level of the prediction
...
# Health Tips Section based on prediction results
...
# Expanders for additional information on health metrics
